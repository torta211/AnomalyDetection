...
from NumentaDetector import NumentaDetector



class NumentaDetectorTM(NumentaDetector):
  """
  ...
  def __init__(self, *args, **kwargs):
    ...
    super(NumentaDetectorTM, self).__init__(*args, **kwargs)


  def initialize(self):
    # Get config params, setting the RDSE resolution
    rangePadding = abs(self.input_max - self.input_min) * 0.2

    modelParams = getScalarMetricWithTimeOfDayAnomalyParams(
      metricData=[0],
      minVal=self.input_min-rangePadding,
      maxVal=self.input_max+rangePadding,
      minResolution=0.001,
    ...
    numentaLearningPeriod = int(math.floor(self.probationary_period / 2.0))
    self.anomaly_likelihood = anomaly_likelihood.AnomalyLikelihood(
      learningPeriod=numentaLearningPeriod,
      estimationSamples=self.probationary_period-numentaLearningPeriod,
